#羚羊云Windows SDK使用说明
##一、概述
本SDK可供Windows平台下的应用调用，为开发者提供接入羚羊视频云的开发接口，使开发者能够轻松实现视频相关的应用。羚羊视频云在视频传输和云存储领域有着领先的开发技术和丰富的产品经验,设计了高质量、宽适应性、分布式、模块化的音视频传输和存储云平台。SDK为上层应用提供简单的API接口，实现直播播放、云端录像播放、消息透传等功能。
...
##五、开发示例
>本章节介绍如何调用SDK的API接口来实现播放器、消息透传的功能。
...
```
>传入startCloudService的参数详见[《API手册》]()。
...
```
该接口可以播放直播流，也可以播放历史流，主要在url上区分，根据传入的url类型的不同会播放不同的流。url格式请参照《API手册》中的“开始播放”功能接口的详细介绍。
